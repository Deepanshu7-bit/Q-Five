...
function VideoWorkCard({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative w-full rounded-3xl overflow-hidden bg-surface-elevated border border-border group-hover:border-accent-crimson transition-all duration-300 shadow-xl",
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div
            className={cn(
              ...
              isHovered ? "scale-110 bg-accent-crimson" : "opacity-90"
            )}
          >
        ...
        <div className="absolute top-4 left-4 pointer-events-none">
          <span className="text-[10px] font-mono uppercase tracking-widest bg-black/75 backdrop-blur-md text-white/95 px-3 py-1 rounded-full border border-white/20">
            {video.category}
          </span>
        </div>
        ...
        <div className="absolute bottom-5 left-5 right-5 text-left pointer-events-none">
          ...
          <p className="text-xs font-mono text-white/80 mt-1 line-clamp-1">
...
export default function VideoEditingWorkPage() {
  ...
  return (
    <div className="pt-28 sm:pt-36 pb-24 bg-background text-foreground transition-colors duration-300">
      {/* Filmstrip Header */}
      <div className="w-full flex justify-center gap-2 sm:gap-3 py-3 border-b border-border-subtle bg-surface select-none overflow-hidden mb-12">
        {Array.from({ length: 28 }).map((_, i) => (
          <div key={i} className="w-3.5 h-5 rounded-sm bg-surface-muted border border-border-strong/40 flex-shrink-0" />
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        ...
        {/* Bottom CTA */}
        <div className="p-8 sm:p-14 rounded-3xl bg-surface border border-border-strong text-center space-y-6 shadow-xl">
      ...
      {/* Filmstrip Footer */}
      <div className="w-full flex justify-center gap-2 sm:gap-3 py-3 border-t border-border-subtle bg-surface select-none overflow-hidden mt-20">
        {Array.from({ length: 28 }).map((_, i) => (
          <div key={i} className="w-3.5 h-5 rounded-sm bg-surface-muted border border-border-strong/40 flex-shrink-0" />
        ))}
      </div>
